function [W visB hidB] = rbm_train(conf,data)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Training RBM                                                       %  
...
for i=1:conf.eNum
    ...
    for j=1:conf.bNum
       ...
       for k=1:conf.gNum
           % down
           vI      = hidNs*W' + repmat(visB,sNum,1);                                 
           visNs = sampling3(vI,rand(size(vI)));           
           % up
    ...
    if ~isempty(conf.vis_dir)
        save_images(strcat(conf.vis_dir,'rbm_rec_',num2str(i),'.mat'),(visNs+1)/2,sNum,conf.row,conf.col);        
        if i==1, save_images(strcat(conf.vis_dir,'rbm_org.mat'),(visP+1)/2,sNum,conf.row,conf.col); end
    end
    %
    if conf.plot_
        mse_plot(i) = mse;
